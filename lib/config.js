...
exports.read = function() {
  ...
  if(fs.existsSync(mupJsonPath)) {
    ...
    //load hooks
    if (mupJson.hooks) {
      var hookNames = ['before-setup', 'after-setup', 'before-deploy', 'after-deploy'];
      validateHooks(mupJson.hooks, hookNames);
      hookNames.map(function (hook) {
        if (mupJson.hooks[hook]) {
          if (!fs.existsSync(mupJson.hooks[hook])) {
            mupErrorLog('Script for hook "'+hook+'" (' + mupJson.hooks[hook] + ') does not exist');
          }
          try {
            var script = fs.readFileSync(mupJson.hooks[hook], 'utf8');
          } catch (err) {
            mupErrorLog('Could not read script for hook "'+hook+'" (' + mupJson.hooks[hook] + '): ' + err);
          }
          mupJson.hooks[hook] = script;
        }
      });
    }
    if(mupJson.ssl) {
...
function validateHooks(hooks, hookNames) {
  Object.keys(hooks).forEach(function (hook) {
    if (hookNames.indexOf(hook) == -1) {
      mupErrorLog('Unknown hook name "' + hook + '"');
    }
  });
}
